...
if opcion == "📊 Reporte Mensual":
    ...
    if not df_mes.empty:
        ...
        # Historial detallado (Formatado a 2 decimales)
        ...
        with col1: st.write("**Metraje Total Acumulado**"); st.bar_chart(data=stats, x="Operador", y="Suma Total (m)", color="#1E88E5")
        with col2: st.write("**Promedio de Eficiencia Diario**"); st.bar_chart(data=stats, x="Operador", y="Promedio Individual (m)", color="#FFC107")
    else:
...
# --- VISTA 2: REGISTRAR ---
    ...
    else: st.warning("🔒 Ingrese contraseña en el menú lateral para registrar.")
    ...
    if tiene_acceso():
        ...
        if not df_raw.empty:
            ...
            if not st.session_state.del_confirm:
                if st.button("🗑️ Eliminar seleccionado"): st.session_state.del_confirm = True; st.rerun()
            else:
